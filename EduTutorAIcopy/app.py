...
os.environ['OAUTHLIB_INSECURE_TRANSPORT'] = '1'  # Fix for InsecureTransportError
...
def diagnostic():
    ...
    set_user_level(student_id, 'beginner')
    return render_template('quiz.html', quiz=quiz)
...
def login():
    flow = google_auth_oauthlib.flow.Flow.from_client_secrets_file(
        ...
        scopes=['https://www.googleapis.com/auth/classroom.courses.readonly',
                ...
                'openid'],
        redirect_uri='http://localhost:5000/callback'  # Ensure port matches app.run()
    )
...
if __name__ == '__main__':
    app.run(debug=True, port=5000)  # Changed to port 5000 (standard Flask port)